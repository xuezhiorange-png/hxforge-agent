...
**M3 sequencing status (recorded at TASK-020 authority reconciliation, post PR #119 / merge `b1b6ea7`):**

- TASK-020's specific capability allocation within the M3 family is NOT frozen on `main`. The TASK-020 number is reserved for the M3 shell-and-tube capability family, but no specific capability (configuration schema foundation, TEMA schema, geometry, etc.) is bound to TASK-020 by this backlog.
- PR #118 (`docs/task-020-shell-and-tube-configuration-schema-design` @ `565082d8cbe4a0ea96a45ba97f8f36d92f7ed060` at the time of authoring this entry) is a Draft design proposal for the M3 Shell-and-Tube Configuration Schema Foundation capability. PR #118 is NOT the backlog's frozen allocation; the Draft allocation is one proposal pending review, independent re-review, Ready authorization, and merge authorization.
- TASK-020 implementation is NOT AUTHORIZED.
- TASK-021 through TASK-039 are NOT allocated to any specific M3 capability. They remain PLANNED / NOT STARTED unless a separate Charles-authorized M3 sequencing amendment round is opened.
- The TASK-020 capability allocation, the TASK-021–TASK-039 capability allocation, and the M3 sequencing timeline are a separate Charles-authorized M3 sequencing round, distinct from this TASK-019 Amendment 002-K corrective round.
- TASK-019 Amendment 002-K (case_02 cost-stack) is an M2 follow-up amendment, NOT an M3 capability. The M3 row above does not include Amendment 002-K scope; Amendment 002-K scope is recorded in the M2 row above.
