...
## 7b. Phase2SourceRecordDescriptor — independent authority artifact
...
class Phase2SourceRecordDescriptor:
    ...
    def verify_or_raise(
        ...
        *,
        ...
        expected_cei = source_record.candidate_evaluation_identity.candidate_evaluation_identity_digest \
        ...
        expected_evidence = verified_evidence.compute_explicit_evidence_digest() \
            if verified_evidence is not None else None
        if self.verified_rating_evidence_digest != expected_evidence:
            raise ValueError("verified_rating_evidence_digest mismatch")
        expected_invalid = source_record.invalid_rating_evidence.invalid_evidence_digest \
        ...
        expected_audit = source_record.claimed_rating_result_audit.audit_digest \
        ...
        expected_failure = source_failure_binding.descriptor_binding_digest \
            if source_failure_binding is not None else None
        if self.evaluation_failure_digest != expected_failure:
            raise ValueError("evaluation_failure_digest mismatch")
        # Self-hash integrity
        for v, n in [(self.identity_snapshot_digest, "identity"),
                      (self.descriptor_digest, "descriptor")]:
            if not self.DIGEST_PATTERN.match(v):
                raise ValueError(f"invalid {n} digest")
        for v, n in [(self.candidate_evaluation_identity_digest, "cei"),
            ...
            if v is not None and not self.DIGEST_PATTERN.match(v):
                raise ValueError(f"invalid {n} digest")
        payload = {"source_qualified_candidate_id": self.source_qualified_candidate_id,
...
def build_phase2_source_record_descriptor(
    *,
    ...
    verified_evidence_digest = verified_evidence.compute_explicit_evidence_digest() \
        if verified_evidence is not None else None
    evaluation_failure_digest = source_failure_binding.descriptor_binding_digest \
        if source_failure_binding is not None else None
    payload = {"source_qualified_candidate_id": source_record.source_qualified_candidate_id,
        ...
        "candidate_evaluation_identity_digest": source_record.candidate_evaluation_identity.candidate_evaluation_identity_digest
            if source_record.candidate_evaluation_identity is not None else None,
        "verified_rating_evidence_digest": verified_evidence_digest,
        "invalid_rating_evidence_digest": source_record.invalid_rating_evidence.invalid_evidence_digest
            if source_record.invalid_rating_evidence is not None else None,
...
class Phase2SourceRecordIdentitySnapshot(BaseModel):
    ...
    def verify_or_raise(
        ...
        *,
        ...
        """Authoritative verifier: validates all 14 fields against source_record."""
        if self.source_qualified_candidate_id != source_record.source_qualified_candidate_id:
            raise ValueError("candidate_id mismatch")
        if self.evaluation_order_index != source_record.evaluation_order_index:
            raise ValueError("evaluation_index mismatch")
        ...
        if self.candidate_evaluation_identity_digest != expected_identity_digest:
            raise ValueError("candidate_evaluation_identity_digest mismatch")
        expected_invalid_digest = source_record.invalid_rating_evidence.invalid_evidence_digest \
            if source_record.invalid_rating_evidence is not None else None
        if self.invalid_rating_evidence_digest != expected_invalid_digest:
            raise ValueError("invalid_rating_evidence_digest mismatch")
        expected_audit_digest = source_record.claimed_rating_result_audit.audit_digest \
            if source_record.claimed_rating_result_audit is not None else None
        if self.claimed_rating_result_audit_digest != expected_audit_digest:
            raise ValueError("claimed_rating_result_audit_digest mismatch")
        # Self-hash replay is already in model validator
...
class Phase2SourceRecordSnapshot(BaseModel):
    ...
    def verify_or_raise(
        ...
        *,
        ...
        if self.candidate_evaluation_identity_digest != expected_identity_digest:
            raise ValueError("candidate_evaluation_identity_digest mismatch")
        expected_verified_digest = verified_evidence.compute_explicit_evidence_digest() \
            if verified_evidence is not None else None
...
class Phase3EvaluationInput(BaseModel):
    ...
    def verify_or_raise(
        ...
        *,
        ...
        if len(source_failure_bindings) != N: raise ValueError("source_failure_bindings length != N")
        if len(evidence_failure_bindings) != N: raise ValueError("evidence_failure_bindings length != N")
        ...
        for i, d in enumerate(phase2_source_record_descriptors):
            ...
            if d is None:
                if expected_desc_digest is not None:
                    raise ValueError(f"[{i}] expected None descriptor digest, got {expected_desc_digest}")
                continue
...
class Phase3SourceRecordBinding(BaseModel):
    ...
    def _validate(self) -> typing.Self:
        ...
        if self.evidence_failure_binding_digest is not None and not self.DIGEST_PATTERN.match(self.evidence_failure_binding_digest):
    ...
    def verify_or_raise(
        ...
        *,
        ...
        expected_ve = verified_evidence.compute_explicit_evidence_digest() if verified_evidence is not None else None
...
PREPARATION_STAGE_MATRIX: dict[Phase3PreparationFailureStage, dict[str, StageFieldRequirement]] = {
    Phase3PreparationFailureStage.WARNING_DESCRIPTOR: {
        ...
        "complete_snapshot": StageFieldRequirement.FORBIDDEN,
    ...
    Phase3PreparationFailureStage.SOURCE_BINDING: {
        "identity_snapshot": StageFieldRequirement.REQUIRED,
        "complete_snapshot": StageFieldRequirement.OPTIONAL_AUTHENTICATED,
...
### 14.2a verify_preparation_stage_matrix
...
def verify_preparation_stage_matrix(
    *,
    ...
    if source_candidate_evaluation_state is not VERIFIED:
        raise ValueError(f"Preparation only valid for VERIFIED source, got {source_candidate_evaluation_state}")
    ...
    for name, rule in rules.items():
        ...
        elif rule is StageFieldRequirement.FORBIDDEN:
            if val is not None:
...
- ALL stages require identity_snapshot (REQUIRED), so identity_snapshot_digest is always present.
- The digests for other fields are None when their artifact is FORBIDDEN or not yet built.

### 14.3 PreparationResult
    ...
    def _validate(self) -> typing.Self:
        ...
        if self.identity_snapshot is not None:
            if self.identity_snapshot.candidate_evaluation_state is not VERIFIED:
                raise ValueError(f"Preparation only valid for VERIFIED source, got {self.identity_snapshot.candidate_evaluation_state}")
        ...
        if self.identity_snapshot_digest != expected_id_digest:
            raise ValueError("identity_snapshot_digest mismatch")
        expected_cs_digest = self.complete_snapshot.snapshot_digest if self.complete_snapshot is not None else None
        if self.complete_snapshot_digest != expected_cs_digest:
            raise ValueError("complete_snapshot_digest mismatch")
        expected_sb_digest = self.source_binding.binding_digest if self.source_binding is not None else None
        if self.source_binding_digest != expected_sb_digest:
            raise ValueError("source_binding_digest mismatch")
        expected_cin_digest = self.classification_input.classification_input_digest if self.classification_input is not None else None
        if self.classification_input_digest != expected_cin_digest:
            raise ValueError("classification_input_digest mismatch")
        expected_efb_digest = self.evidence_failure_binding.descriptor_binding_digest if self.evidence_failure_binding is not None else None
        if self.evidence_failure_binding_digest != expected_efb_digest:
            raise ValueError("evidence_failure_binding_digest mismatch")
        expected_sfb_digest = self.source_failure_binding.descriptor_binding_digest if self.source_failure_binding is not None else None
        if self.source_failure_binding_digest != expected_sfb_digest:
            raise ValueError("source_failure_binding_digest mismatch")
        expected_p3fb_digest = self.phase3_failure_binding.descriptor_binding_digest if self.phase3_failure_binding is not None else None
    ...
    def verify_or_raise(
        ...
        *,
        ...
        if source_record.candidate_evaluation_state is not VERIFIED:
            raise ValueError(f"Preparation only valid for VERIFIED source, got {source_record.candidate_evaluation_state}")
...
def build_phase3_candidate_preparation_result(
    *,
    ...
    return Phase3CandidatePreparationResult(
        ...
        evidence_failure_binding=evidence_failure_binding, source_failure_binding=source_failure_binding,
        phase3_failure_binding=phase3_failure_binding,
...
def disposition_from_preparation_failure(
    *,
    ...
    if preparation_result.phase3_failure_binding_digest != phase3_failure_binding.descriptor_binding_digest:
        raise ValueError("disposition_from_preparation_failure: external binding digest != stored binding digest")
    # Validate canonical fields match
    if preparation_result.phase3_failure_binding.original_code != phase3_failure_binding.original_code:
        raise ValueError("disposition_from_preparation_failure: original_code mismatch")
    if preparation_result.phase3_failure_binding.payload_digest != phase3_failure_binding.payload_digest:
        raise ValueError("disposition_from_preparation_failure: payload_digest mismatch")
    if preparation_result.phase3_failure_binding.canonicalization_error_digest != phase3_failure_binding.canonicalization_error_digest:
    ...
    if phase3_failure_binding_digest is None:
        raise ValueError("disposition_from_preparation_failure: no binding digest (canonicalization broken)")
...
class CandidateDispositionRecord(BaseModel):
    ...
    def _validate(self) -> typing.Self:
        ...
        # RUNTIME_FAILED
            ...
            if self.failure_origin == PHASE2_EVALUATION:
                ...
                if self.phase3_failure_binding_digest is None: raise ValueError("RF(P3): phase3 failure binding required")
    ...
    def verify_or_raise(
        ...
        *,
        ...
        if self.candidate_evaluation_identity_digest != expected_identity_digest:
            raise ValueError("candidate_evaluation_identity_digest mismatch")
        expected_evidence_digest = source_record.verified_rating_evidence.compute_explicit_evidence_digest() \
    ...
    *,
    ...
    if rec.rating_status is None:
        return _phase3_runtime(rec, eb, PHASE3_MISSING_RATING_STATUS, "No rating status.",
            ...
            warning_descriptors=warning_descriptors, blocker_descriptors=blocker_descriptors,
            source_failure_binding=source_failure_binding, evidence_failure_binding=evidence_failure_binding)
    if rec.rating_status == "blocked":
        ...
        if vf is not None:
            return _phase3_runtime_from_validation(rec, eb, vf, source_identity_record_descriptor_digest=sid,
                source_record_descriptor_digest=scd, warning_descriptors=warning_descriptors,
                blocker_descriptors=blocker_descriptors)
...
def build_optimization_result(
    *,
    ...
    return (OptimizationResult(
        ...
        result_hash=result_hash), graph)

    # Authoritative replay before return — builder must not return
    # a result that hasn't passed independent authority replay
    ...
    # Provenance already verified via builder's own graph construction;
    # external provenance verifier receives independent authority at call site.
    return (result, graph)
...
# === Model validator / authoritative verifier boundary ===
#
# OptimizationResult._validate() (model validator) guarantees:
#   - schema version
#   - count non-negativity
#   - count sum invariants
#   - tuple exact lengths
#   - digest format (sha256:* or None for nullable tuples)
#   - Top-N prefix of ranked
#   - result core self-hash
#   - envelope hash
#   - UUID
#
# OptimizationResult.verify_or_raise() (authoritative verifier) guarantees:
#   - source-state positional nullability (VERIFIED → prep digest non-None,
#     non-VERIFIED → prep digest None)
#   - preparation digest equality with independent preparation_result artifact
#   - source-binding digest equality with independent source_binding artifact
#   - disposition/ranking/provenance semantic consistency
#
# Model validation is structural/self-integrity only.
# Source-state semantic acceptance REQUIRES verify_or_raise()
# with independent authority artifacts.
...
def verify_optimization_result_or_raise(
    ...
    for i, (rec, cand) in enumerate(zip(ei.evaluation_records, ei.materialization_result.candidates)):
        ...
        for j, (d, b) in enumerate(zip(wdt, wbt)):
            ...
            if d.original_code != b.original_code: raise ValueError(f"[{i}] warn[{j}] code mismatch")
            if d.message_payload_digest != b.message_payload_digest: raise ValueError(f"[{i}] warn[{j}] digest mismatch")
        if len(bdt) != len(bbt): raise ValueError(f"[{i}] blocker descriptor/binding count mismatch")
        for j, (d, b) in enumerate(zip(bdt, bbt)):
...
def verify_phase3_provenance_graph_or_raise(graph, *, ei, dispositions, ranked,
    ...
    source_records: tuple[CandidateEvaluationRecord, ...] | None = None,
    preparation_results: tuple[Phase3CandidatePreparationResult | None, ...] | None = None,
    source_bindings: tuple[Phase3SourceRecordBinding | None, ...] | None = None,
):
    N = total_candidate_count
    # 0) Independent authority validation — nullable tuples must derive from artifacts
    if source_records is not None and preparation_results is not None and source_bindings is not None:
...
| V-P3-WARNING-DESCRIPTOR-FAIL | VERIFIED | FORBIDDEN | REQUIRED | FORBIDDEN | FORBIDDEN | REQUIRED | FAILED | WARNING_DESCRIPTOR | REQUIRED | FORBIDDEN | REQUIRED | sha256:… | None | PHASE3_CLASSIFICATION | RUNTIME_FAILED |
| V-P3-BLOCKER-DESCRIPTOR-FAIL | VERIFIED | FORBIDDEN | REQUIRED | FORBIDDEN | FORBIDDEN | REQUIRED | FAILED | BLOCKER_DESCRIPTOR | REQUIRED | FORBIDDEN | REQUIRED | sha256:… | None | PHASE3_CLASSIFICATION | RUNTIME_FAILED |
| V-P3-FAILURE-DESCRIPTOR-FAIL | VERIFIED | FORBIDDEN | REQUIRED | FORBIDDEN | FORBIDDEN | REQUIRED | FAILED | FAILURE_DESCRIPTOR | REQUIRED | FORBIDDEN | REQUIRED | sha256:… | None | PHASE3_CLASSIFICATION | RUNTIME_FAILED |
| V-P3-EVIDENCE-DIGEST-FAIL | VERIFIED | FORBIDDEN | REQUIRED | FORBIDDEN | FORBIDDEN | REQUIRED | FAILED | EVIDENCE_DIGEST | REQUIRED | FORBIDDEN | REQUIRED | sha256:… | None | PHASE3_CLASSIFICATION | RUNTIME_FAILED |
| V-P3-SOURCE-BINDING-FAIL | VERIFIED | OPTIONAL_AUTHENTICATED | REQUIRED | REQUIRED | FORBIDDEN | REQUIRED | FAILED | SOURCE_BINDING | REQUIRED | FORBIDDEN | REQUIRED | sha256:… | sha256:… | PHASE3_CLASSIFICATION | RUNTIME_FAILED |
