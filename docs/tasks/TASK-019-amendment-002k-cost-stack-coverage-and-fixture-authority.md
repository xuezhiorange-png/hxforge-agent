...
| Field | Value |
|---|---|
| **Design-content provenance** | PRESERVED FROM PR #119 (PR #119 HEAD COMMIT `aa6692eb5a978b3ab3ebd052aed50bcd9b03a213`), WHICH MERGED THE SAME DESIGN CONTENT UNDER A SUPERSEDED TASK-020 LABEL (`docs/tasks/TASK-020-cost-stack-coverage-and-fixture-authority.md`, superseded by this corrective rename); PR #119 MERGE COMMIT `b1b6ea7f5b766d171f0454a29f3b9a6ba05907b1`; DESIGN CONTENT IDENTICAL TO THE PR #119 MERGED VERSION WITH ONLY IDENTITY / NUMBERING / HEADER CORRECTIONS PER TASK-019 AMENDMENT 002-K CORRECTIVE DRAFT |
| **Corrective identity status** | TASK-019 DESIGN AMENDMENT 002-K RE-HOME DRAFT / NOT MERGED |
| **Implementation status** | NOT AUTHORIZED |
| **Source authority** | §17 (TASK-019 Design Amendment 002-J) merged into `main`; case_02 002-G mass-chain already merged; case_03 002-H / 002-I bridge patterns already merged; §17A.x (this contract, TASK-019 Design Amendment 002-K, design-content identical to PR #119 / merge `b1b6ea7` design content re-homed under the correct TASK-019 amendment letter 002-K) |
...
2. **Merged / frozen repository design contract** (`main` @ `b1b6ea7`; §15, §16, §17, 002-A / 002-D / 002-E / 002-F / 002-G).
...
this amendment declares the following **target** state for the case_02 cost-record selection. These are **contract targets**, not implementation commitments; the implementation is gated on Slice A + Slice B per §17A.8.
...
| `REQUIRED_MISSING` | Field is required by §17A.5.2 / §17.2 but the central value is not yet present in the case_02 fixture. | Slice C (after Slice B's production-chain execution). | NO (only derived from production chain) |
...
The following table enumerates every expected_output field that §17A contractually addresses. Fields already at `EXISTING_FROZEN` are listed for completeness; the table does NOT re-author them. Fields at `REQUIRED_MISSING` / `DERIVED_VIA_CHAIN` / `DEFERRED` / `PROHIBITED_MANUAL` are the future Slice targets.
...
Per §17.7 item 2 and §17A.10, the following is the **only** allowed write sequence for any `REQUIRED_MISSING` / `DERIVED_VIA_CHAIN` / `PROHIBITED_MANUAL` field:
...
Any future round that wishes to populate a `REQUIRED_MISSING` / `DERIVED_VIA_CHAIN` / `PROHIBITED_MANUAL` expected_output field on case_02 MUST follow this exact sequence, in this exact order. Skipping any step or reordering any step is a §17A.10 violation.
...
The above items belong to the M3 family per `docs/TASK_BACKLOG.md` line 59 ("TASK-020 through TASK-039 cover TEMA configuration schemas, tube layout, shell diameter, tube-side rating, Kern screening, Bell–Delaware, pressure-drop decomposition, thermal expansion screening, preliminary mechanical boundaries, materials, costing, optimization, API, report and Golden validation"). They are NOT TASK-020's responsibility; they belong to separately authorized future cost-model scope outside TASK-019 Design Amendment 002-K, task number unassigned. (The phrase 'TASK-020' in the immediately preceding backlog quote is a historical, superseded, non-authoritative reference to the original PR #119 / merge `b1b6ea7` filename and is not current task authority.)
...
§17A.7.2 is a §17A.10 anti-fabrication guard. Violation is a hard STOP. The future separately authorized future cost-model scope outside TASK-019 Design Amendment 002-K, task number unassigned design cards for thermal / pressure-drop are the correct place for this content.
...
The implementation authorized under this amendment is split into **four slices**, each with explicit **entry conditions** and **exit conditions**. Each slice is a separate Charles-authorized round. No slice may begin until the previous slice's exit conditions are met. No slice may combine with another slice.
...
| Entry conditions | (1) this amendment is merged into `main`. (2) §17 is merged into `main`. (3) Charles explicitly authorizes Slice A. |
...
| R9 | case_02 `input.cost_records_bridge` (the sub-block itself) | Required by §17A.4.1 and §17A.3.2 | Slice A (cannot start Slice A without the contract-frozen sub-block shape) | Slice A authoring | MUST NOT be invented under this amendment; MUST be authored in Slice A | `REPO_SOURCE_REQUIRED` (via Slice A) |
| R10 | case_02 `input.cost_model_selection` (the sub-block itself) | Required by §17A.3.2 (case_03 pattern: `currency_ISO_4217`, `date_ISO_8601`, `escalation_rule_id`, `region_id`) | Slice A | Slice A authoring | MUST NOT be invented under this amendment; MUST be authored in Slice A | `REPO_SOURCE_REQUIRED` (via Slice A) |
| R11 | case_02 `input.lifecycle_inputs` (the sub-block itself) | Required by §17A.5.2 (`life_cycle_energy_envelope.life_cycle_energy_summary.*`) | Slice A | Slice A authoring | MUST NOT be invented under this amendment; MUST be authored in Slice A | `REPO_SOURCE_REQUIRED` (via Slice A) |
...
| R22 | case_02 expected_output authority for `provenance.*` (record provenance / selection trace) | Required by §17A.4.1 | Slice A | Slice A authoring (`provenance_amendment_id` per record) | MUST NOT be invented; MUST be a stable case-bound identifier | `REPO_SOURCE_REQUIRED` (via Slice A) |
...
| R24 | TASK-019 Design Amendment 002-K tracking GitHub Issue | Required for Slice A / B / C / D tracking | Slice A entry condition #3 (Charles explicit authorization) | Future Charles-authorized round | MUST NOT be opened under §17A itself; MUST be opened in a separate Charles-authorized round | `CHARLES_SOURCE_REQUIRED` |
| R25 | Pressure-drop / thermal-method / TEMA / Kern / Bell-Delaware implementation authority | Excluded from this amendment (per §17A.7); belongs to separately authorized future cost-model scope outside TASK-019 Design Amendment 002-K, task number unassigned | n/a (out of this amendment scope) | Future separately authorized future cost-model scope outside TASK-019 Design Amendment 002-K, task number unassigned design card | MUST NOT be authored under this amendment; MUST be a separate Charles-authorized round | `DEFERRED` |

**Status transition rule (binding)**: a register item's status MAY transition from `SOURCE_MISSING` / `REPO_SOURCE_REQUIRED` / `CHARLES_SOURCE_REQUIRED` to `RESOLVED` ONLY in the round that actually produces the missing authority. The status transition MUST be recorded in `_provenance_metadata.json` and the §17A.9 register (in a future round, not this round). A `RESOLVED` item in this round's register is a §17A.10 anti-fabrication violation.
...
- **§17A design document does NOT authorize implementation.** this amendment is a design contract. Any implementation under this amendment alone is a §17A.10 violation. Implementation requires Slice A → B → C → D, each with its own Charles-authorized round.
- **A skeleton is NOT an implementation approval.** §17A.8's four Slices are skeletons, not approvals. Each Slice's entry condition #3 ("Charles explicitly authorizes Slice N") is a hard gate.
- **A missing field is NOT an invitation to supply a default.** A `REQUIRED_MISSING` field in §17A.5.2 stays missing until the production chain run in Slice C captures the value. No default, no placeholder, no first-valid-record, no copy from a sibling case.
- **The current fixture subset is NOT a complete catalog.** case_02 currently has mass-chain bridges (002-F / 002-G) but no cost-stack bridges. The absence of a cost-stack bridge is NOT a license to invent one under §17A. The cost-stack bridge is gated on Slice A.
...
- **A documentation example is NOT `expected_output` authority.** Numbers appearing in this amendment / §17 / §15 / §16 prose are examples, not authority. Authority is the production chain output captured in Slice C.
...
2. **No TODO-as-authorized-scope**: any `TODO` / `FIXME` / `placeholder` / `to-be-determined` in a Slice A / B / C / D commit MUST be flagged as a §17A.10 violation. Slice A / B / C / D commits MUST be either complete or explicitly aborted.
3. **No pressure-drop / Bell-Delaware / Kern / TEMA / thermal implementation content**: per §17A.7. Any such content in a TASK-019 Design Amendment 002-K lineage commit is a hard STOP.
4. **No runtime catalog scan authorization**: per §17A.4.2. Any code that performs a runtime catalog scan in a TASK-019 Design Amendment 002-K lineage commit is a hard STOP.
...
7. **No fixture contract test regression**: the hard-coded `test_expected_output_unchanged_across_adapter_runs` (asserting `fixture_02["expected_output"]["mass_kg"]["fluid_mass_kg"] == 1.05`) MUST NOT regress under any TASK-019 Design Amendment 002-K lineage change.
...
- A `RESOLVED` status in the §17A.9 register that was authored in the same round (i.e. the round both added the missing authority and marked the item `RESOLVED`).
- A `expected_output` numeric value in a TASK-019 Design Amendment 002-K lineage commit that is NOT traceable to a real production chain run.
- A pressure-drop / Bell-Delaware / Kern / TEMA / thermal implementation in a TASK-019 Design Amendment 002-K lineage commit.
- A runtime catalog scan / resolver in a TASK-019 Design Amendment 002-K lineage commit.
- A combined-slice commit (e.g. one commit that includes both Slice A and Slice B content).
- A TASK-019 Design Amendment 002-K lineage commit that does not name the Slice (A / B / C / D) in the commit message subject.

---

## this amendment Change Log (binding)

| Date | Change | Author |
|---|---|---|
| 2026-07-10 | Initial this amendment design contract authored (this file). All 10 sections (§17A.1 – §17A.10) created from the TASK-019 frozen design cross-reference. Status: DESIGN FROZEN / MERGE-NOT-AUTHORIZED. | Charles-authorized design-only round |
...
> **This is a corrective re-home of design content originally merged via PR #119 under a superseded TASK-020 label (historical, non-authoritative).**
> **IMPLEMENTATION NOT AUTHORIZED.**
> **FIXTURE MUTATION NOT AUTHORIZED.**
> **EXPECTED OUTPUT VALUES NOT INVENTED.**
> **NO REMOTE MUTATION PERFORMED.**

This document is a **design contract** (governance-only). It does not authorize production implementation, fixture mutation, test mutation, or any algorithm authoring. The implementation is gated on Slices A → B → C → D per §17A.8, each requiring its own Charles-authorized round.
