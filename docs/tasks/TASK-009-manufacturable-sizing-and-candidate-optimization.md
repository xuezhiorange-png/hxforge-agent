...
| 42 | 4806697790 | CHANGES REQUIRED |
| 43 | 4806775891 | CHANGES REQUIRED |
| 44 | 4806817422 | CHANGES REQUIRED |
| 45 | 4806876116 | CHANGES REQUIRED |
| 46 | 4806969354 | CHANGES REQUIRED |
| 47 | 4807086031 | CHANGES REQUIRED |
| 48 | 4807137816 | CHANGES REQUIRED |
| 49 | 4807231368 | CHANGES REQUIRED |
| 50 | 4807305227 | CHANGES REQUIRED |
| 51 | 4807382934 | CHANGES REQUIRED |
| 52 | 4807462753 | CHANGES REQUIRED |
| 53 | 4807683216 | PASS |
...
### 14.7.2 Canonicalization core and integration contracts

### 14.7.2.1 Block manifest

| Block ID | Marker | Status | Python execution | strict mypy |
|----------|--------|--------|------------------|-------------|
| `TASK009_CANONICALIZATION_CORE_PY` | `BEGIN/END TASK009_CANONICALIZATION_CORE_PY` | executable normative contract | required on 3.11 and 3.12 | required on 3.11 and 3.12 |
| `TASK009_CANONICALIZATION_INTEGRATION_PSEUDOCODE` | `BEGIN/END TASK009_CANONICALIZATION_INTEGRATION_PSEUDOCODE` | normative typed design pseudocode | not claimed | not claimed |

Only blocks with status `executable normative contract` enter the mechanical Python/mypy gate.
The pseudocode block is a design reference and must not be extracted or verified as executable code.
...
<!-- BEGIN TASK009_CANONICALIZATION_INTEGRATION_PSEUDOCODE -->
```python
...
```
<!-- END TASK009_CANONICALIZATION_INTEGRATION_PSEUDOCODE -->



### 14.7.2.4 Mechanical extraction and verification contract

#### Extraction rules (byte-preserving)

1. The document is read as UTF-8 bytes. No text normalisation is applied before extraction.
2. Each `BEGIN` marker and its corresponding `END` marker must each appear exactly once in the document.
...
9. No helpers, fixtures or definitions are appended.
10. After writing to `/tmp`, the file is re-read and bytes are verified to be identical.
11. The SHA-256 digest of the extracted bytes is recorded and reused for all verification steps.
12. Python execution and mypy must use the same file with the same SHA-256.

#### Authoritative extraction script
...
def extract_exact_block(document_bytes: bytes) -> bytes:
    if document_bytes.count(BEGIN_MARKER) != 1:
        raise RuntimeError(
            "core BEGIN marker count must equal 1"
        )
    if document_bytes.count(END_MARKER) != 1:
        raise RuntimeError(
            "core END marker count must equal 1"
        )
    begin_index = (
    ...
    if not region.startswith(OPENING_FENCE):
        raise RuntimeError(
            "BEGIN marker must be followed immediately "
            "by a python fence"
        )
    ...
    if not code:
        raise RuntimeError(
            "extracted core block must not be empty"
        )
    return code


document_bytes = DOCUMENT_PATH.read_bytes()
extracted_bytes = extract_exact_block(document_bytes)
...
DOCUMENT_EXTRACTOR_PATH="/tmp/document_task009_core_extractor.py"
...
if not region_lines:
    raise RuntimeError(
        "extractor marked region must not be empty"
    )
if region_lines[0] != opening_fence:
    raise RuntimeError(
        "extractor BEGIN marker must be followed "
        "immediately by a python fence"
    )
if region_lines[-1] != closing_fence:
    raise RuntimeError(
        "extractor END marker must be preceded "
        "immediately by a closing fence"
    )
internal_fence_lines = [
line
for line in region_lines[1:-1]
if line.startswith(b"```")
]
if internal_fence_lines:
    raise RuntimeError(
        "extractor marked region must not contain "
        "internal markdown fence lines"
    )
extracted_bytes = b"".join(
    region_lines[1:-1]
)
...
cmp --silent \
  "${DOCUMENT_EXTRACTOR_PATH}" \
  "${EXTRACTOR_PATH}"
DOCUMENT_EXTRACTOR_SHA="$(
  sha256sum "${DOCUMENT_EXTRACTOR_PATH}" \
  | awk '{print $1}'
)"
...
10. Python 3.12 is mandatory. If Python 3.12 is not available or fails, the pipeline fails:
    - `TASK-009: BLOCKED`
    - `Not ready for Round 53 Engineering Design Review`
...
The extracted core extractor script itself must also compile without warnings:
...
501. **Input**: Review history table after Round 53 approval. **Expected output**: Round 53 row exists with Comment ID `4807683216`. **Exception**: N/A — sync test: structural consistency check.
502. **Input**: Round 53 review history row. **Expected output**: Decision cell reads `PASS`. **Exception**: N/A — sync test: structural consistency check.
503. **Input**: Round 53 row in review history. **Expected output**: Round 53 immediately follows Round 52. Every Review History row contains exactly three non-empty cells. **Exception**: N/A — sync test: ordering and structure check.
504. **Input**: Status text at top of document. **Expected output**: Documents that Round 53 passed and implementation is approved. **Exception**: N/A — sync test: gate reference check.
...
528. **Input**: Integration block marker name. **Expected output**: Integration marker is `TASK009_CANONICALIZATION_INTEGRATION_PSEUDOCODE` (not `_PY`). **Exception**: N/A — psuedocode naming contract.
529. **Input**: Exact extracted core block via byte-preserving extraction. **Expected output**: `python3.11 -m mypy --strict /tmp/task009_canonicalization_core.py` reports `Success: no issues found in 1 source file`. **Exception**: N/A — strict mypy passes on 3.11.
530. **Input**: Exact extracted core block via byte-preserving extraction. **Expected output**: `python3.12 -m mypy --strict /tmp/task009_canonicalization_core.py` reports `Success: no issues found in 1 source file`. **Exception**: N/A — strict mypy passes on 3.12.
...
539. **Input**: Acceptance Criteria labels #501–#540 as Round 30. **Expected output**: Acceptance Criteria records Round 53 Engineering Design Review as passed. **Exception**: N/A — sync test: label and gate consistency.
540. **Input**: Current document and Issue synchronization state after Round 53 approval. **Expected output**: Review History contains Round 53 and Comment ID 4807683216; Document documents that Round 53 passed and implementation is approved; Issue Frozen SHA equals approved contract SHA 7e4522ab5be740fb6af759743c1c1f79801312fc; Issue status is READY; TASK-009 may proceed to implementation. **Exception**: N/A — structural synchronization contract.
---
...
556. **Input**: Remove `from pydantic import BaseModel` from a copy of the extracted file. **Expected output**: `mypy --strict` fails with a `name-defined` error for `BaseModel`. **Exception**: exit code ≠ 0.
...
564. **Input**: The sole authoritative verification pipeline with failure cleanup trap, `TASK009_CORE_EXTRACTOR_PY` marker region, isolated mutated document copies, and a malicious pre-existing untrusted helper. **Expected output**:
- The untrusted helper is deleted before any Python execution and is never invoked.
- The extractor is derived directly from the frozen document by the quoted inline bootstrap.
- The first marker-region line is exactly the Python opening fence.
- The final marker-region line is exactly the closing fence.
- No internal line beginning with a Markdown fence token is accepted.
- An extra standalone opening fence fails closed.
- An extra standalone closing fence fails closed.
- Premature close followed by reopen fails closed.
...
1. Round 53 Engineering Design Review passed.
...
- [x] Round 53 Engineering Design Review passed before implementation starts
...
- [ ] Required test matrix entries 1–565 (continuous), including Round 6 (89–136), Round 7 (137–176), Round 8 (177–204), Round 9 (205–244), Round 10 (245–295), Round 11 (296–332), Round 12 (333–370), Round 13 (371–405), Round 14 (406–430), Round 15 (431–460), Round 16 (461–500), Round 30 (501–540), and Round 33 (541–565)
