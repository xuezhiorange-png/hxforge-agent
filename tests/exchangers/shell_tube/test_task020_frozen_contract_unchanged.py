...
def test_design_contract_present() -> None:
    assert _DESIGN_CONTRACT_PATH.exists(), (
        f"missing design contract: {_DESIGN_CONTRACT_PATH}"
    )
...
def test_design_contract_first_heading_present() -> None:
    ...
    assert _EXPECTED_DESIGN_HEADING in _DESIGN_CONTRACT_PATH.read_text(
        encoding="utf-8"
