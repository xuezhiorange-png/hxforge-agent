...
def test_v06_construction_family_overlay_is_explicit_and_hash_neutral() -> None:
    assert SUPPORTED_CONSTRUCTION_FAMILIES == (
        "FIXED_TUBESHEET",
        "U_TUBE",
        "FLOATING_HEAD",
    )
    # The v0.6 overlay is outside the legacy formula-authority projection.
    assert recompute_engineering_authority_hash() == (
        "50c829a3b12a0e2082de4255e7b81b921759b0f2e393e57ada561c50b58a105d"
    )


@pytest.mark.parametrize("family", SUPPORTED_CONSTRUCTION_FAMILIES)
def test_v06_families_are_admitted_without_formula_branch(
    ...
    assert candidate.geometry.engineering_authority_hash == ENGINEERING_AUTHORITY_HASH
...
def test_fixed_tubesheet_result_identity_remains_pinned() -> None:
    ...
    assert result.geometry.engineering_authority_hash == ENGINEERING_AUTHORITY_HASH


def test_unknown_construction_family_remains_fail_closed() -> None:
    result = validate_request(_family_fixture("UNSUPPORTED"))
