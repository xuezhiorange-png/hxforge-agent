#!/usr/bin/env python3
"""
Case 1: Counterflow water-water double-pipe heat exchanger rating.
...
# ====================================================================
# Gnielinski correlation (matching production TubeTurbulentGnielinski
# and AnnulusTurbulentGnielinskiDH)
# ====================================================================
...
def compute_thermal_resistance(
    ...
    Returns dict with r_conv_inner, r_foul_inner, r_wall, r_foul_outer,
    r_conv_outer, total_resistance, ua_w_k.
    """
...
def evaluate_residual(
    ...
    Returns dict with all computed intermediate values.
    """
    # --- Trial outlet enthalpies (matching production lines 1652-1653) ---
    ...
    # --- Bulk temperatures (production line 1820-1821) ---
    ...
    # --- Tube side (hot fluid in tube for this case) ---
    # Flow area: π/4 × D_i²
    ...
    # D_char for tube = D_i (inside_diameter basis)
    h_tube = Nu_tube * props_hot["k"] / dh_tube
    ...
    # D_char for annulus = D_h = D_outer - D_o (hydraulic_diameter basis)
    h_annulus = Nu_annulus * props_cold["k"] / dh_annulus
...
def bisect_solve(
    ...
    Bisects on residual = Q - UA(Q) × LMTD(Q).
    """
    q_low = 0.0
    q_high = q_max
...
def main():
    print("=" * 70)
    print("Case 1: Counterflow water-water double-pipe rating")
    print("=" * 70)
    ...
    # Estimate Q_max (independent enthalpy limits for counterflow)
    # Hot limit: T_hot_out_min = T_cold_in + 0.5 K
    ...
    # Cold limit: T_cold_out_max = T_hot_in - 0.5 K
    ...
    reference_data = {
        "case": "case1_counterflow",
        "description": "Counterflow water-water double-pipe heat exchanger",
        "geometry": {
            "D_i_m": D_i,
            "D_o_m": D_o,
            "D_outer_m": D_outer,
        ...
        "reference": {
            ...
            "derivation_file": "case1_counterflow_reference.py",
    ...
    json_path = os.path.join(script_dir, "case1_reference_result.json")
