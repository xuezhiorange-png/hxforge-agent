...
| 5 | **No fouling in reference (production may differ)** | The reference scripts include the fouling resistances from the geometry specification (`R_f,i` and `R_f,o`). However, the production kernel may apply additional fouling logic, correction factors, or different default fouling values that are not mirrored here. |
